...
import (
	...
	"github.com/graph-guard/ggproxy/pkg/engine/playmon/internal/constrcheck/internal/test"
	...
	for _, do := range d {
		...
		t.Run(strings.TrimSuffix(fileName, ".yml"), func(t *testing.T) {
			...
			run := func(t *testing.T, opr *gqt.Operation) {
				...
				// variables := make(map[string][]gqlparse.Token, len(ts.Inputs))
				// for path, v := range ts.Inputs {
				// 	variables[path] = v.Tokens
				// }
